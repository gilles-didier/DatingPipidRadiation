...
void fillUncertaintyLogRankData(int node, TypeTree *tree, TypeUncertaintyLogRankData *lr) {
	...
	if(tree->node[node].child == NOSUCH) {
		...
		if(tree->node[tree->node[node].child].sibling == NOSUCH) {
			printf("Problem node %d\n", node);
			exit(0);
		}
...
void fillLogDistribution(TypeDistribution *logD, double *logCond, int n, TypeTree *tree, TypeFossilFeature *fos, TypeModelParam *param) {
	...
	for(i = 0; i < basicU->size; i++)
		if(basicU->node[i].child == NOSUCH && basicU->time[i] == NO_TIME)
			warning("Execution warning %d %s (Uncertainty.c:319)\n", i, tree->name[i]);
	if(basicU->parent == NULL)
		setParent(basicU);
...
double getLogLikelihoodTreeFossil(TypeTree *tree, TypeFossilFeature *fos, TypeModelParam *param) {
	...
	for(l = 0; l < size; l++) {
		...
		if(isnan(tmp) || isinf(tmp)) {
			error("Error getLogLikelihoodTreeFossil  of tree %d (%d) l 532\ntime %lf %lf %lf\n", l, treeList[l]->size, treeList[l]->minTimeInt.inf, treeList[l]->minTimeInt.sup, treeList[l]->minTime);
		}
		res += tmp;
...
double getLogIntProbTypeA(double a, double b, double maxTime, int k, TypeUncertaintyLikeParameters *param) {
	return ((double)k) * log(param->sampling) - log(param->birth) - log((double)k) + log(pow((1. - exp(param->omega * (maxTime - a))) / (param->bs - param->as * exp(param->omega * (maxTime - a))), (double)k) - pow((1. - exp(param->omega * (maxTime - b))) / (param->bs - param->as * exp(param->omega * (maxTime - b))), (double)k));
}
...
double getLogIntProbTypeC(double a, double b, double endTime, double maxTime, int k, int l, TypeUncertaintyLikeParameters *param) {
	return ((double)k - l) * myLog(param->bs, -param->as * exp(param->omega * (maxTime - endTime))) + ((double)l) * myLog((1 - param->alpha) * param->bs, -(1 - param->beta) * param->as * exp(param->omega * (maxTime - endTime))) - log((double)k) - log(param->birth) + log(pow((1 - exp(param->omega * (endTime - a))) / (param->bs - param->as * exp(param->omega * (maxTime - a))), k) - pow((1 - exp(param->omega * (endTime - b))) / (param->bs - param->as * exp(param->omega * (maxTime - b))), k));
}

double getLogIntProbTypeD(double a, double b, double endTime, double maxTime, int k, TypeUncertaintyLikeParameters *param) {
	return ((double)k) * (log(param->bs - param->as * exp(param->omega * (maxTime - endTime))) - log(param->bma)) - log((double)k) - log(param->birth) + log(pow((1 - exp(param->omega * (endTime - a))) / (param->bs - param->as * exp(param->omega * (maxTime - a))), k) - pow((1 - exp(param->omega * (endTime - b))) / (param->bs - param->as * exp(param->omega * (maxTime - b))), k));
}

/*return p(k, startTime)*/
double getLogIntProbTypeB(double a, double b, double endTime, double maxTime, int k, TypeUncertaintyLikeParameters *param) {
	return +log(param->fossil) - log(param->birth) - log((double)k + 1.) + ((double)k + 1.) * (log(param->bs - param->as * exp(param->omega * (maxTime - endTime))) - log(param->bma)) + log(pow((1. - exp(param->omega * (endTime - a))) / (param->bs - param->as * exp(param->omega * (maxTime - a))), (double)k + 1.) - pow((1. - exp(param->omega * (endTime - b))) / (param->bs - param->as * exp(param->omega * (maxTime - b))), (double)k + 1.));
}
...
double getLogProbTypeA(double startTime, double maxTime, int k, TypeUncertaintyLikeParameters *param) {
	...
	return ((double)k) * log(param->sampling) + 2. * log(param->bma) + param->omega * time + ((double)k - 1) * log(1. - exp(param->omega * time)) - ((double)k + 1) * log(param->bs - param->as * exp(param->omega * time));
}
...
double getLogProbTypeB(double startTime, double endTime, double maxTime, int k, TypeUncertaintyLikeParameters *param) {
	if(startTime == endTime) {
		...
		return log(param->fossil) + param->omega * (endTime - startTime) + ((double)k) * (log(1 - exp(param->omega * (endTime - startTime))) - log(param->bma)) + ((double)k + 2.) * (log(param->bs - param->as * exp(param->omega * (maxTime - endTime))) - log(param->bs - param->as * exp(param->omega * (maxTime - startTime))));
}
...
double getLogProbTypeC(double startTime, double endTime, double maxTime, int k, int l, TypeUncertaintyLikeParameters *param) {
	return param->omega * (endTime - startTime) + ((double)k - 1.) * (myLog(1., -exp(param->omega * (endTime - startTime))) - log(param->bma)) - ((double)k + 1.) * myLog(param->bs, -param->as * exp(param->omega * (maxTime - startTime))) + ((double)k - l + 1.) * myLog(param->bs, -param->as * exp(param->omega * (maxTime - endTime))) + ((double)l) * myLog((1 - param->alpha) * param->bs, -(1 - param->beta) * param->as * exp(param->omega * (maxTime - endTime)));
}
...
double getLogProbTypeD(double startTime, double endTime, double maxTime, int k, TypeUncertaintyLikeParameters *param) {
	return param->omega * (endTime - startTime) + ((double)k - 1.) * (myLog(1., -exp(param->omega * (endTime - startTime))) - log(param->bma)) + ((double)k + 1.) * (myLog(param->bs, -param->as * exp(param->omega * (maxTime - endTime))) - myLog(param->bs, -param->as * exp(param->omega * (maxTime - startTime))));
}
	...
	if(tree->node[n].child == NOSUCH) {
		...
		if(tree->time[n] != NO_TIME)
			...
			for(c = tree->node[tree->node[n].child].sibling; c != NOSUCH; c = tree->node[c].sibling)
				...
				else if(tree->time[indexMin[c]] == tree->time[indexMin[n]] && tree->node[indexMin[c]].child != NOSUCH)
					indexMin[n] = indexMin[c];
		}
	}
}
