...
typedef struct TMP_REORDER_SIZE_NAME {
	char *name;
	int size;
	int index;
} TypeTmpReorderSizeName;
...
static int compareTmpReorderSizeName(const void *a, const void *b);
static void reorderTreeSizeNameRec(int *tmpSize, char **tmpName, int n, TypeTree *tree);
...
void fillTips(int n, TypeTree *tree, char **tab, int *size) {
	if(tree->node[n].child == NOSUCH) {
		tab[*size] = tree->name[n];
		(*size)++;
	} else {
		int c;
		for(c = tree->node[n].child; c != NOSUCH; c = tree->node[c].sibling)
			fillTips(c, tree, tab, size);
	}
}

void fprintTips(FILE *f, int n, TypeTree *tree) {
	if(tree->node[n].child == NOSUCH) {
...
int compareTmpReorderSizeName(const void *a, const void *b) {
	if(((TypeTmpReorderSizeName *)a)->size > ((TypeTmpReorderSizeName *)b)->size)
		return 1;
	if(((TypeTmpReorderSizeName *)a)->size < ((TypeTmpReorderSizeName *)b)->size)
		return -1;
	return strcmp(((TypeTmpReorderSizeName *)a)->name, ((TypeTmpReorderSizeName *)b)->name);
}

void reorderTreeSizeNameRec(int *tmpSize, char **tmpName, int n, TypeTree *tree) {
	int c, nchild = 0;
	for(c = tree->node[n].child; c != NOSUCH; c = tree->node[c].sibling) {
		nchild++;
		reorderTreeSizeNameRec(tmpSize, tmpName, c, tree);
	}
	if(nchild > 0) {
		int i;
		TypeTmpReorderSizeName *tab;
		tab = (TypeTmpReorderSizeName *)malloc(nchild * sizeof(TypeTmpReorderSizeName));
		nchild = 0;
		for(c = tree->node[n].child; c != NOSUCH; c = tree->node[c].sibling) {
			tab[nchild].size = tmpSize[c];
			tab[nchild].name = tmpName[c];
			tab[nchild].index = c;
			nchild++;
		}
		qsort(tab, nchild, sizeof(TypeTmpReorderSizeName), compareTmpReorderSizeName);
		...
		for(c = tree->node[n].child; c != NOSUCH; c = tree->node[c].sibling)
			tmpSize[n] += tmpSize[c];
		tmpName[n] = tmpName[tree->node[n].child];
		for(c = tree->node[tree->node[n].child].sibling; c != NOSUCH; c = tree->node[c].sibling)
			if(strcmp(tmpName[n], tmpName[c]) > 0)
				tmpName[n] = tmpName[c];

	} else {
		tmpSize[n] = 1;
		if(tree->name != NULL)
			tmpName[n] = tree->name[n];
		else
			tmpName[n] = NULL;
	}
}
...
void reorderTreeSizeName(TypeTree *tree) {
	int *tmpSize;
	char **tmpName;
	if(tree == NULL)
		return;
	tmpSize = (int *)malloc(tree->size * sizeof(int));
	tmpName = (char **)malloc(tree->size * sizeof(char *));
	reorderTreeSizeNameRec(tmpSize, tmpName, tree->root, tree);
	free((void *)tmpSize);
	free((void *)tmpName);
}
...
int isBinary(int n, TypeTree *tree) {
	if(tree->node[n].child == NOSUCH)
		return 1;
	...
	return isBinary(tree->node[n].child, tree) && isBinary(tree->node[tree->node[n].child].sibling, tree);
}
...
void fprintTreeX(FILE *f, TypeTree *tree) {
	...
	if(tree->size == 0)
		...
		for(n = 0; n < tree->size; n++) {
			if(tree->name != NULL && tree->name[n])
				fprintf(f, "%d (%s)", n, tree->name[n]);
			else
				fprintf(f, "%d", n);
			if(tree->time != NULL) {
				if(tree->time[n] == NO_TIME)
...
int iterateBinary(int n, TypeTree *resT, TypeTree *tree) {
	...
	for(m = n; tree->node[m].child != NOSUCH && tree->node[tree->node[m].child].sibling == NOSUCH; m = tree->node[m].child)
		if(tree->time) {
			if(tree->time[m] != NO_TIME)
				resT->time[resT->size] += tree->time[m];
			else
				resT->time[resT->size] = NO_TIME;
		}
...
void printNodeDebug(FILE *f, int s, int depth, TypeTree *tree, char **name) {
	...
	if(depth >= 0) {
		...
		if(name != NULL && name[s] != NULL)
			...
			fprintf(f, "%d", s);
		int c, nc = 0;
		for(c = tree->node[s].child; c != NOSUCH; c = tree->node[c].sibling)
			nc++;
		fprintf(f, " (%d) ", nc);
...
void reindexTree(TypeTree *tree, int *index) {
	...
	for(n = 0; n < tree->size; n++) {
		...
		if(tree->time != NULL)
			newtime[index[n]] = tree->time[n];
		if(tree->parent != NULL)
			newparent[index[n]] = index[tree->parent[n]];
		if(tree->name != NULL)
...
/*make the set of trees 'tree'to be binary and such that each node name has the same index in each of the tree, the nodes with names have the first indexes */
void fixTreeSet(TypeTree **tree) {
	...
	for(i = 0; tree[i] != NULL; i++) {
		int n;
		if(!isBinary(tree[i]->root, tree[i])) {
			TypeTree *old = tree[i];
			tree[i] = fixBinary(old);
			freeTree(old);
		}
		toBinary(tree[i]);
		if(tree[i]->name != NULL) {
...
/*make the set of trees 'tree'to be binary and such that each node name has the same index in each of the tree, the nodes with names have the first indexes */
void fixTreeSetBis(TypeTree **tree) {
	int i, nName;
	TypeLexiTree *dict;
	dict = newLexiTree();
	nName = 0;
