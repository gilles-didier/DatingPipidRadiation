...
double logProbObsFBD(double t, double maxTime, TypeModelCoeffFBD *coeff) {
	return log(coeff->ba - coeff->ab * exp(-coeff->omega * (maxTime - t))) - log(coeff->beta - coeff->alpha * exp(-coeff->omega * (maxTime - t)));
}

double logProbNotObsFBD(double t, double maxTime, TypeModelCoeffFBD *coeff) {
	return log(coeff->alpha) + log(coeff->beta) + log(1 - exp(-coeff->omega * (maxTime - t))) - log(coeff->beta - coeff->alpha * exp(-coeff->omega * (maxTime - t)));
}

double logDensWaitFBD(double s, double e, double maxTime, int n, TypeModelCoeffFBD *coeff) {
	return log((double)n) + ((double)n - 1) * log(coeff->beta - coeff->alpha * exp(-coeff->omega * (maxTime - e))) - ((double)n) * (coeff->fba * (e - s) + log(coeff->beta - coeff->alpha * exp(-coeff->omega * (maxTime - s)))) + log(coeff->bfba - coeff->afbb * exp(-coeff->omega * (maxTime - e)));
}
...
double logProbNotObsBD(double t, double maxTime, TypeModelCoeffBD *coeff) {
	return log(coeff->death) + log(1. - exp(-coeff->bd * (maxTime - t))) - log(coeff->birth - coeff->death * exp(-coeff->bd * (maxTime - t)));
}

double logDensWaitBD(double s, double e, double maxTime, int n, TypeModelCoeffBD *coeff) {
	return log((double)n) + log(coeff->birth) + log(coeff->bd) - ((double)n) * coeff->bd * (e - s) + ((double)n - 1) * log(coeff->birth - coeff->death * exp(-coeff->bd * (maxTime - e))) - ((double)n) * log(coeff->birth - coeff->death * exp(-coeff->bd * (maxTime - s)));
}

double logDistWaitBD(double s, double e, double maxTime, int n, TypeModelCoeffBD *coeff) {
	return -((double)n) * coeff->bd * (e - s) + ((double)n) * (log(coeff->birth - coeff->death * exp(-coeff->bd * (maxTime - e))) - log(coeff->birth - coeff->death * exp(-coeff->bd * (maxTime - s))));
}
...
double getLogLikelihoodStadler(TypeTree *tree, TypeFossilFeature *fos, TypeModelParam *param) {
	...
	ll = ((double)n) * log(param->birth) + ((double)fos->size) * log(param->fossil) + ((double)e) * log(4.) - log_q_Stadler(tree->minTime, c1, c2, param);
	for(i = 0; i < n; i++)
...
double logDistWaitOneFBD(double b, double e, double maxTime, TypeModelCoeffFBD *coeff) {
	return -coeff->fba * (e - b) + log(coeff->beta - coeff->alpha * exp(-coeff->omega * (maxTime - e))) - log(coeff->beta - coeff->alpha * exp(-coeff->omega * (maxTime - b)));
}
...
double logDensWaitOneFBD(double b, double e, double maxTime, TypeModelCoeffFBD *coeff) {
	return -coeff->fba * (e - b) - log(coeff->beta - coeff->alpha * exp(-coeff->omega * (maxTime - b)));
}
