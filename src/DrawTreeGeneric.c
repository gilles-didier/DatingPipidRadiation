...
void drawScaleGeneric(double x, double y, TypeInfoDrawTreeGeneric *info) {
	...
	printf("Draw scale %lf - %lf\n", info->param.tmin, info->param.tmax);
	...
	for(cur = start; cur <= end; cur += step) {
		...
		if(flag)
			sprintf(tmp, "%.1lf", fabs(cur));
		else
			sprintf(tmp, "%.0lf", fabs(cur));
		info->funct.drawText(x + (cur - info->param.tmin) * info->param.scale, y + info->param.tickLength, tmp, "t", &(info->param));
	}
	info->funct.drawText(x, y + 10 * info->param.tickLength, "Million of years ago", "l", &(info->param));
}
...
void drawTreeGeneric(TypeTree *tree, TypeInfoDrawTreeGeneric *info, TypeAdditionalDrawTreeGeneric *add, TypeAdditionalDrawTreeGenericGeneral *addG) {
	...
	//		info->funct.drawText((tree->time[tree->root]-info->param.tmin)*info->param.scale+info->param.xoffset+info->param.labelSep, ytab[tree->root]+info->param.yoffset, tree->name[tree->root], "l", &(info->param));
	{
		if(tree->node[tree->root].child == NOSUCH)
			...
			info->funct.drawText((tree->time[tree->root] - info->param.tmin) * info->param.scale + info->param.xoffset + info->param.labelSep, ytab[tree->root] + info->param.yoffset, tree->name[tree->root], "tr", &(info->param));
	}
	if(add != NULL)
	...
	if(tree->node[n].child != NOSUCH) {
		...
		//			info->funct.drawText((tree->time[n]-info->param.tmin)*info->param.scale+info->param.xoffset+info->param.labelSep, y+info->param.yoffset, tree->name[n], "l", &(info->param));
		{
			if(tree->node[n].child == NOSUCH)
				...
				info->funct.drawText((tree->time[n] - info->param.tmin) * info->param.scale + info->param.xoffset + info->param.labelSep, y + info->param.yoffset, tree->name[n], "tr", &(info->param));
		}
		// else {
		// char tmp[100];
		// sprintf(tmp, "%d", n);
		// info->funct.drawText((tree->time[n]-info->param.tmin)*info->param.scale+info->param.xoffset+info->param.labelSep, y+info->param.yoffset, tmp, "l", &(info->param));
		//}
	} else {
