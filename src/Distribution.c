...
double getMedianDens(TypeDistribution d) {
	int i;
	double sum = 0.;
	for(i = 1; i < d.size && sum <= 0.5; i++)
		sum += d.item[i].dens * (d.item[i].val - d.item[i - 1].val);
...
double getMedian(TypeDistribution d) {
	int i;
	double sum = 0., sumTmp = 0.;
	for(i = 0; i < d.size; i++)
		sum += d.item[i].dens;
	if(d.size > 1) {
		for(i = 0; i < d.size && sumTmp <= 0.5 * sum; i++)
			sumTmp += d.item[i].dens;
		;
		return d.item[i - 1].val;
	} else {
		if(d.size > 0)
...
double getQuantileDensity(TypeDistribution d, double q) {
	...
	if(d.size <= 1)
		return 0;
	for(i = 0; i < d.size && sumTmp <= q * sum; i++)
		sumTmp += d.item[i].dens;
	;
	return d.item[i - 1].val;
}
