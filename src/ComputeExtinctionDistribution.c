...
int main(int argc, char **argv) {
	...
	if((fi = fopen(inputFileNameTree, "r")) && (ff = fopen(inputFileNameFossil, "r"))) {
		...
		minFossilTime = getMinFossilIntTime(fos);
		...
		for(i = 0; tree[i] != NULL; i++) {
			...
			if(minTimeIntSup == NO_TIME)
				tree[i]->minTimeInt.sup = tree[i]->minTimeInt.inf;
		...
		*/
		...
		sprintf(outputDistribution, "%sproba.txt", outputPrefix);
		if((fo = fopen(outputDistribution, "w"))) {
			char *tmp;
			fprintf(fo, "Probability that extinction time > 272.3\n");
			for(j = 0; j < nClade; j++) {
				int k;
				if(strrchr(nameClade[j], '/') != NULL)
					...
					tmp = nameClade[j];
				k = binarySearchValDistribution(-272.3, dist[nTaxa + j]);
				fprintf(fo, "%s\t%le\n", tmp, 1. - dist[nTaxa + j].item[k].dens);
			}
			fclose(fo);
		}
		...
		for(j = 0; j < nTaxa; j++) {
			sprintf(outputDistribution, "%s%s.csv", outputPrefix, nameTaxa[j]);
			if((fo = fopen(outputDistribution, "w"))) {
				if(outDens) {
		...
		for(j = 0; j < nClade; j++) {
			...
			if(strrchr(nameClade[j], '/') != NULL)
				...
				tmp = nameClade[j];
			sprintf(outputDistribution, "%s%s.csv", outputPrefix, tmp);
			if((fo = fopen(outputDistribution, "w"))) {
				if(outDens) {
		...
		int *whatClade, *others;
		others = (int *)malloc(tree[0]->size * sizeof(int));
		whatClade = (int *)malloc(tree[0]->size * sizeof(int));
		for(n = 0; n < tree[0]->size; n++)
			whatClade[n] = nClade + 1;
		for(j = 0; j < nClade; j++)
			for(i = 0; clade[nTaxa + j][i] != END_LIST_INT; i++)
				whatClade[clade[nTaxa + j][i]] = j + 1;
		n = 0;
		for(j = 0; j < nTaxa; j++)
			if(whatClade[j] == nClade + 1)
				others[n++] = j;
		others[n++] = END_LIST_INT;
		sprintf(outputDistribution, "%s.gnu", outputPrefix);
		if((fo = fopen(outputDistribution, "w"))) {
			fprintf(fo, "set terminal tikz color size 10,8 font \",18\" createstyle;\nset xtics 20\nunset ytics\nunset key\nset xrange [120:0]\nset arrow from 272.3, graph 0 to 272.3, graph 1 nohead\n\nset border linewidth 4\n\nset style fill transparent solid 0.9 border\nfilter(x,min,max) = (x > min && x < max) ? x : 1/0\n\n");
			for(j = 0; j < nTaxa; j++) {
				int k;
				...
				for(k = 0; nameTaxa[j][k] != '\0'; k++)
					if(nameTaxa[j][k] == ' ')
						nameTaxa[j][k] = '_';
				fprintf(fo, "set output \"%s_FigTikzTaxa%s.tex\"\nplot '%s_%s.csv' using(filter(abs($1), %lf,%lf)):2 with filledcurves x1 lc %d notitle, '' using (abs($1)):2 with lines notitle lw 4 lc %d\nset output\n\n", outputPrefix, nameTaxa[j], outputPrefix, nameTaxa[j], -dist[j].item[dist[j].size - 1].val, -quant[j], whatClade[clade[j][0]], whatClade[clade[j][0]]);
			}
			if(nClade > 0) {
				char *tmp;
				int k;
		...
		if(nClade > 0) {
			...
			for(j = 0; j < nClade; j++) {
				if(strrchr(nameClade[j], '/') != NULL)
					...
					tmp = nameClade[j];
				sprintf(outputDistribution, "%s_Clade_%s.tex", outputPrefix, tmp);
				if((fo = fopen(outputDistribution, "w"))) {
					...
					for(k = 0; clade[nTaxa + j][k] != END_LIST_INT; k++) {
						fprintf(fo, "\\input %s_FigTikzTaxa%s", outputPrefix, nameTaxa[clade[nTaxa + j][k]]);
						if((k + 1) % 4 != 0)
							fprintf(fo, " & ");
						else
							fprintf(fo, " \\\\\n");
			...
			sprintf(outputDistribution, "%s_Clade_Others.tex", outputPrefix);
			if((fo = fopen(outputDistribution, "w"))) {
				fprintf(fo, "\\documentclass[border=10pt,svgnames]{standalone} \n\\usepackage{verbatim}\n\\usepackage{xcolor}\n\\usepackage{amssymb}\n\\usepackage{array,booktabs,colortbl}\n\\usepackage{tikz}\n\\usetikzlibrary{shapes,snakes,arrows,decorations.markings,calc,positioning,shadows.blur}\n\\usepackage{gnuplot-lua-tikz}\n\\usepackage{amsmath,amsfonts,amssymb,amsxtra,amsthm,amstext}\n\\renewcommand*\\familydefault{\\sfdefault}\n\\usepackage{sfmath}\n\\begin{document}\n\\pagestyle{empty}\n\n\\begin{tabular}{cccc}");
				for(k = 0; others[k] != END_LIST_INT; k++) {
					fprintf(fo, "\\input %s_FigTikzTaxa%s", outputPrefix, nameTaxa[others[k]]);
					if((k + 1) % 4 != 0)
						fprintf(fo, " & ");
					else
						fprintf(fo, " \\\\\n");
		...
		qsort((void *)nameTaxa, nTaxa, sizeof(char *), compareString);
		...
		if((fo = fopen(outputDistribution, "w"))) {
			...
			for(j = 0; j < nTaxa; j++) {
				...
				// if(nameTaxa[j][k] == ' ')
				// nameTaxa[j][k] = '_';
				fprintf(fo, "\\input %s_FigTikzTaxa%s", outputPrefix, nameTaxa[j]);
				if((j + 1) % 5 != 0)
					fprintf(fo, " & ");
				else
					fprintf(fo, " \\\\\n");
