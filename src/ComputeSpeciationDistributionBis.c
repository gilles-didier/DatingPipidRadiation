...
int main(int argc, char **argv) {
	...
	if((fi = fopen(inputFileNameTree, "r")) && (fl = fopen(inputFileNameList, "r")) && (ff = fopen(inputFileNameFossil, "r"))) {
		...
		int i, sizeTree, sizeTreeAll, *node;
		char **list;
		FILE *fout, *find;
		...
		printf("%d trees\n", i);
		for(sizeTreeAll = 0; tree[sizeTreeAll] != NULL; sizeTreeAll++)
			;
		node = (int *)malloc(sizeTreeAll * sizeof(int));
		sizeTree = 0;
		for(i = 0; tree[i] != NULL; i++) {
			int n;
		...
		printf("%d/%d have the clade\n", sizeTree, sizeTreeAll);
		tree[sizeTree] = NULL;
		...
		if((fout = fopen(nameOutput, "w")) && (find = fopen(nameIndex, "w"))) {
			dist = MCMCSamplingSpeciationDist(fout, find, node, tree, fos, step, al, nBurn, nGap, nSamp, prop, &windSize, &init, probSpe, probExt);
			fclose(fout);
			fclose(find);
		} else
			error("Cannot open %s or %s\n", nameOutput, nameIndex);
		...
		sprintf(outputDistribution, "%s_mode.txt", outputPrefix);
		if((fo = fopen(outputDistribution, "w"))) {
			fprintf(fo, "mode %lf\n", getMode(dist));
			fclose(fo);
		}
