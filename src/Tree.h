...
/*Read a table of pairwise node times.*/
...
/*Return a binary copy of a possibly multifurcating tree.*/
...
/*Read all Newick trees from a stream.*/
TypeTree **readTrees(FILE *f);
/*Reorder children by subtree size.*/
void reorderTreeSize(TypeTree *tree);
/*Reorder children by subtree size and name.*/
void reorderTreeSizeName(TypeTree *tree);
/*Apply a node index remapping to a tree.*/
void reindexTree(TypeTree *tree, int *index);
/*Fill a node index remapping by traversing a tree.*/
