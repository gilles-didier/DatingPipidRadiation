...
int main(int argc, char **argv) {
	char *inputFileNameTree, *inputFileNameFossil = NULL, *tmp, outputFileNameG[SIZE_BUFFER_CHAR], *outputFileName, option[256], format = '1', *nameOutput = "/home/gilles/Dropbox/Alfred/fig.gnu", *prefixClade = "/home/gilles/Dropbox/Extinction/data/Pipid_trees_analysenoncontrainte.tre_", *suffixClade = ".txt", *prefixDist = NULL, *suffixDist = ".csv", **tip = NULL;
	FILE *fi;
	int i, j, debug = 0;
	double minTimeIntInf = NO_TIME, maxTimeIntSup = 0., minDisplayTime = DBL_MAX, maxDisplayTime = 0., treeScaleStep = 10., figwidth = 100.;
	TypeMap *map = NULL;
	for(i = 0; i < 256; i++)
		option[i] = 0;
	...
	for(i = 1; i < argc && *(argv[i]) == '-'; i++) {
		...
		if(option['b']) {
			...
			if((i + 1) < argc && (fi = fopen(argv[++i], "r"))) {
				tip = readList(fi);
				int j;
				printf("Reading tip file:\n");
				for(j = 0; tip[j] != NULL; j++)
					printf("%s\n", tip[j]);
				fclose(fi);
			} else {
				error("Error while reading %s.\n", argv[i]);
		...
		if(option['m']) {
			option['m'] = 0;
			if((i + 1) < argc && sscanf(argv[i + 1], "%lf", &(minDisplayTime)) == 1)
				i++;
			else
	...
	if((fi = fopen(inputFileNameTree, "r"))) {
			...
			if((fo = fopen(nameOutput, "w"))) {
				...
				for(i = 0; map[i].key != NULL; i++) {
					...
					if((fi = fopen(nameTmp, "r"))) {
						...
						if(node != NOSUCH) {
							if(tree->node[node].child != NOSUCH && tree->name[node] != NULL)
		...
		if(tip != NULL) {
			...
			FILE *fo;
			printf("tip not null\n");
			if((fo = fopen(nameOutput, "a"))) {
				char *tmpList[2];
				tmpList[1] = NULL;
				fprintf(fo, "\nset style fill transparent solid 0.25 border\nfilter(x,min,max) = (x > min && x < max) ? x : 1/0\n\nset terminal tikz color size 10,6 font \",6\" createstyle;\nset xtics 10\nunset ytics\nunset xlabel\nset key top left\nset output \"FigTikzExt_dist.tex\"\nset xrange [175:25]\nplot");
				for(i = 0; tip[i] != NULL; i++) {
					FILE *fi;
					char nameTmp[STRING_SIZE];
					int node;
					tmpList[0] = tip[i];
					node = getClade(tmpList, tree);
					printf("Clade %s -> node %d\n", tip[i], node);
					sprintf(nameTmp, "%s%s%s", prefixDist, tip[i], suffixDist);
					if((fi = fopen(nameTmp, "r"))) {
						d[node] = readDistribution(fi);
						fclose(fi);
						if(i < 8)
							fprintf(fo, " '%s' using (abs($1)):2 with lines lw 2 title \"%s\",", nameTmp, tip[i]);
						else
							fprintf(fo, " '%s' using (abs($1)):2 with lines lw 2 dashtype \".\" title \"%s\",", nameTmp, tip[i]);
					} else
						fprintf(stderr, "cannot read the file %s\n", nameTmp);
				}
				fclose(fo);
		...
		if((tmp = strrchr(outputFileName, '.')) != NULL)
			tmp[0] = '\0';
		printf("OutputFile : %s\n", outputFileName);
		info.param.tmin = (minDisplayTime != DBL_MAX) ? minDisplayTime : tree->minTime;
		printf("T min %lf\n", info.param.tmin);
...
int compareMap(const void *a, const void *b) {
	if(((TypeMap *)a)->val == NULL) {
		if(((TypeMap *)b)->val == NULL)
			return 0;
		else
			return -1;
	} else {
